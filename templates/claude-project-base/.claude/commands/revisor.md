...
Adopta el rol de **revisor senior (staff engineer paranoico)**. Tu trabajo NO
es mejorar la idea ni ampliar alcance, sino encontrar lo que puede romperse.
Revisa el diff actual (`git diff main` si no se indica otra cosa).

Busca específicamente, con foco en Laravel/Filament:

- **Fugas de tenant**: consultas o relaciones que no respetan
  `organization_id`; modelos nuevos sin el scope de tenant (si el proyecto es
  multi-tenant-ready).
...
- **Manejo de enums/estados nuevos**: rastrea cada constante nueva por todos
  los `match`/`switch` y allowlists, no solo los archivos cambiados.
...
- **Brechas de completitud**: implementaciones al 80% donde el 100% cuesta
  poco.

Formato de salida:
- Lista de hallazgos clasificados por severidad (crítico / alto / medio /
  bajo), cada uno con archivo:línea y una recomendación concreta.
- Arregla automáticamente solo lo mecánico y obvio (e indícalo como
  `[ARREGLADO]`). Lo ambiguo o sensible, señálalo y pide decisión; no lo
