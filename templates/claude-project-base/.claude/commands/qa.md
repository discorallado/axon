...
3. Identifica casos borde NO cubiertos y escribe tests Pest que los cubran
   (feature tests para flujos de usuario, unit tests para lógica de
   dominio).
...
5. Verifica reglas de negocio críticas del dominio: integridad de
   `organization_id` (si aplica), transiciones de estado válidas, permisos
   por rol, unicidad de identificadores.

Modos (según lo que se pida):
- **Rápido**: solo correr la suite y reportar verde/rojo.
- **Completo**: explorar flujos, fenómenos borde, y documentar 5-10
  hallazgos bien evidenciados con pasos para reproducir.
- **Regresión**: correr completo y comparar contra el estado previo.

Salida: reporte con health score aproximado, top de issues por severidad y
pasos de reproducción. Si arreglas algo, hazlo con commits atómicos.
